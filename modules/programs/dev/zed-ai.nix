{lib, ...}: {
  config = {
    ...
    flake.modules.homeManager.zed-ai = {
      ...
      aiEnabled = osConfig.aspects.services.ai.enable or false;
      zedEnabled = osConfig.aspects.programs.zed.enable or false;
    ...
    in
      lib.mkIf (aiEnabled && zedEnabled) {
        home.packages = [echoPython];

        programs.zsh.shellAliases = let
          echoPath = osConfig.aspects.services.ai.echoProjectPath;
        in {
