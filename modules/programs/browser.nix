{inputs, ...}: {
  config = {
    ...
    in
      lib.mkIf (osConfig ? aspects.programs.browser && osConfig.aspects.programs.browser.enable) {
        home.packages = lib.optionals (!isDarwin) [
          inputs.zen-browser.packages."${pkgs.stdenv.hostPlatform.system}".default
        ];

        home.file = lib.mkIf (!isDarwin) (let
          profile = "${osConfig.aspects.programs.browser.profileId}.Default Profile";
        in {
