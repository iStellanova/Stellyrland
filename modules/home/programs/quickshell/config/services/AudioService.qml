            ...
            onStreamFinished: {
                try {
                    ...
                    for (let newItem of newData) {
                        ...
                        // Try matching by id first, then by pwId from the script
                        let nativeNode = root.getAppNode(newItem.id) || (newItem.pwId && newItem.pwId !== -1 ? root.getAppNode(newItem.pwId) : null)
                        ...
                        if (idx !== -1) {
                            ...
                            itemToAdd.pwId = nativeNode ? nativeNode.id : (newItem.pwId || -1)
                            appVolumesModel.append(itemToAdd)
                        }
                    }
