...
Singleton {
    ...
    readonly property string configVersion: "1.3.0"
    ...
    property int nixCurrentGen: 0
    property string nixStoreSize: "0G"
    property string nixStatus: "OK"
    property string nixUpdates: "Not checked"
    property bool nixChecking: false
    
    // ── Popup States & Offsets ────────────────────────────
    // Using a single object for easier iteration and management
    property var popups: ({
        cc:       { visible: false, offset: 0 },
        nc:       { visible: false, offset: 0 },
        cal:      { visible: false, offset: 0 },
        traffic:  { visible: false, offset: 0 },
        ram:      { visible: false, offset: 0 },
        cpu:      { visible: false, offset: 0 },
        gpu:      { visible: false, offset: 0 },
        temp:     { visible: false, offset: 0 },
        media:    { visible: false, offset: 0 },
        nix:      { visible: false, offset: 0 },
        mic:      { visible: false, offset: 0 },
        volume:   { visible: false, offset: 0 },
    ...
    function setPopupVisible(name, visible) {
        if (popups[name]) {
            let p = Object.assign({}, popups)
            p[name] = Object.assign({}, p[name], { visible: visible })
            popups = p
        }
    }

    function updatePopups(changes) {
        let p = Object.assign({}, popups)
        let modified = false
        for (let name in changes) {
            if (p[name] && p[name].visible !== changes[name]) {
                p[name] = Object.assign({}, p[name], { visible: changes[name] })
                modified = true
            }
        }
        if (modified) popups = p
    }

    function setPopupOffset(name, offset) {
        if (popups[name]) {
            let p = Object.assign({}, popups)
            p[name] = Object.assign({}, p[name], { offset: offset })
            popups = p
        }
    }

    function isPopupVisible(name) {
        return popups[name] ? popups[name].visible : false
    }

    function getPopupOffset(name) {
        return popups[name] ? (popups[name].offset || 0) : 0
    }

    // Aliases to maintain signals for specific logic
    readonly property bool nixVisible: isPopupVisible("nix")
    readonly property bool shortcutsVisible: isPopupVisible("shortcuts")
    ...
    // ── Nix Monitor Service ────────────────────────────────
    function refreshNixStats() {
        nixStatsProc.running = true
    }

    function checkNixUpdates() {
        if (nixChecking) return
        nixChecking = true
        nixUpdateCheckProc.running = true
    }

    Process {
        id: nixStatsProc
        command: ["bash", root.scriptsDir + "/get_nix_stats.sh"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                try {
                    ...
                    root.nixGenerations = data.generations
                    root.nixCurrentGen = data.current_generation
                    root.nixStoreSize = data.store_size
                    root.nixStatus = data.status
                } catch (e) { console.error("Nix Stats Parse Error:", e) }
            }
        }
    }

    Process {
        id: nixUpdateCheckProc
        command: ["bash", root.scriptsDir + "/get_nix_stats.sh", "--check-updates"]
        running: false
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                try {
                    ...
                    root.nixCurrentGen = data.current_generation
                    root.nixStoreSize = data.store_size
                    root.nixUpdates = data.updates
                    root.nixStatus = data.status
                } catch (e) { console.error("Nix Updates Parse Error:", e) }
            }
        }
    }
    Timer { 
        interval: ConfigService.pollUpdates
        running: true
        repeat: true
        triggeredOnStart: true
        onTriggered: root.refreshNixStats()
    }

    // Faster polling when the user is actually looking at the stats
    Timer {
        interval: 5000 // 5 seconds
        running: root.isPopupVisible("nix")
        repeat: true
        onTriggered: root.refreshNixStats()
    }
