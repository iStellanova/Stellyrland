...
fetch_data() {
    ...
    if command -v pactl >/dev/null && pactl info >/dev/null 2>&1; then
        pactl --format=json list sink-inputs 2>/dev/null | jq -c '.[] | {
            id: .index,
            pwId: (.properties["object.id"] // -1 | tonumber),
            ...
            volume: ((.volume | to_entries | .[0].value.value_percent // "0%") | sub("%"; "") | tonumber),
            muted: .mute
        }' && return 0
    fi
...
fetch_data | while read -r item; do
    # Extract all relevant fields safely in one jq call
    {
        ...
        read -r volume
        read -r muted
    } < <(echo "$item" | jq -r '.id, .pwId, (.pid // ""), .name, (.icon // ""), (.volume // ""), (.muted // "")')
    ...
    jq -n -c --argjson id "$id" --argjson pwId "$pwId" --arg name "$name" --arg icon "$icon" \
          --argjson volume "$volume" --argjson muted "$muted" \
          '{$id, $pwId, $name, $icon, $volume, $muted}'
done | jq -s '.'
