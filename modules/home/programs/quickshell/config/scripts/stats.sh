...
get_cpu() {
  ...
  while read -r name user nice system idle iowait irq softirq steal guest guest_nice; do
    [[ $name =~ ^cpu[0-9]* ]] || continue
    
    # Total ticks includes everything. 
    # Note: On many systems, user already includes guest, and nice includes guest_nice.
    # Summing all fields provided ensures we have the full denominator.
    ...
    if [[ -n "${prev_total[$name]}" && "${prev_total[$name]}" -ne 0 ]]; then
      ...
      local diff_idle=$((idle_total - prev_idle[$name]))
    ...
    prev_idle[$name]=$idle_total
  done < "$stat_file"

  local cores_json=$(printf ", %s" "${core_usages[@]}")
  echo "\"cpu\": $total_usage, \"cpu_cores\": [${cores_json:2}]"
}

get_cpu_speed() {
  local freq_path="/sys/devices/system/cpu/cpufreq/policy0/scaling_cur_freq"
  local mhz=0
  if [ -f "$freq_path" ]; then
...
get_ram_data() {
  # Optimized single awk pass for all ram stats
  free -b | awk '/^Mem/ {
    used=$3; total=$2; avail=$7; free=$4; cached=$6;
    perc=int(100 * used / total);
    printf "%d %.2f %.2f %.2f %.2f", perc, used/1073741824, avail/1073741824, free/1073741824, cached/1073741824
  }'
}
...
get_gpu_info() {
  ...
  # Combined VRAM awk call
  local vram_gb=$(awk "BEGIN {printf \"%.2f %.2f\", $vram_used/1073741824, $vram_total/1073741824}")
  read -r v_used v_total <<< "$vram_gb"

  echo "\"gpu\": $usage, \"gpu_temp\": $temp, \"gpu_vram_used\": \"$v_used\", \"gpu_vram_total\": \"$v_total\""
}
...
get_net() {
  [ -z "$NET_INTERFACE" ] && echo "0 0" && return
  awk "\$1 ~ \"$NET_INTERFACE\" {print \$2 \" \" \$10}" /proc/net/dev
}
...
while true; do
  ...
  read -r ram_perc ram_gb ram_avail_gb ram_free_gb ram_cached_gb < <(get_ram_data)
  gpu_info=$(get_gpu_info)
  temp=$(get_temp)
  read -r rx_curr tx_curr < <(get_net)
  
  # Combined network awk call
  read -r rx_kbps tx_kbps < <(awk "BEGIN {printf \"%.1f %.1f\", ($rx_curr - $rx_prev) / $INTERVAL / 1024, ($tx_curr - $tx_prev) / $INTERVAL / 1024}")
