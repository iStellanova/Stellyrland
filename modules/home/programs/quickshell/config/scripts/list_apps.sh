...
find -L ~/.local/share/applications ~/.nix-profile/share/applications /etc/profiles/per-user/$USER/share/applications /run/current-system/sw/share/applications /var/lib/flatpak/exports/share/applications /usr/share/applications -name "*.desktop" -type f 2>/dev/null -print0 | xargs -0 awk -F'=' '
BEGIN {
    in_entry = 0
}
