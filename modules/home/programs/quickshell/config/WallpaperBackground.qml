...
PanelWindow {
    ...
    // ── Persistent Shared Circle Mask ──
    Rectangle {
        id: circleMask
        width: root.width; height: root.height
        visible: false
        layer.enabled: true
        color: root.transitionType === "circleShrink" ? "black" : "transparent"
        Rectangle {
            anchors.centerIn: parent
            width: (root.transitionType === "circleGrow") ? (root.diagonal * root.wipeProgress) : (root.diagonal * (1.0 - root.wipeProgress))
            height: width
            radius: width / 2
            color: root.transitionType === "circleShrink" ? "transparent" : "black"
        }
    }
        ...
        Item {
            ...
            VideoOutput {
                ...
                Image {
                    ...
                    cache: false
                    onStatusChanged: {
                        if (status === Image.Ready && root.isTransitioning && root.activeLayer === 1 && !wipeInAnim.running) {
                            proceedWithWipe()
                        }
                    }
                }
        ...
        Item {
            ...
            VideoOutput {
                ...
                Image {
                    ...
                    cache: false
                    onStatusChanged: {
                        if (status === Image.Ready && root.isTransitioning && root.activeLayer === 0 && !wipeInAnim.running) {
                            proceedWithWipe()
                        }
                    }
                }
    ...
    function transitionTo(path, isVideo, framePath) {
        ...
        if (root.activeLayer === 0) {
            ...
            if (isVideo) { 
                playerB.source = path
                playerB.play() 
                if (playerB.mediaStatus >= MediaPlayer.LoadedMedia) proceedWithWipe()
            } else {
                if (imageB.status === Image.Ready) proceedWithWipe()
            }
        } else {
            ...
            if (isVideo) { 
                playerA.source = path
                playerA.play() 
                if (playerA.mediaStatus >= MediaPlayer.LoadedMedia) proceedWithWipe()
            } else {
                if (imageA.status === Image.Ready) proceedWithWipe()
            }
        }
